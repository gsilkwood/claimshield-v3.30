...
export function Step4PreAccidentComps({ data, onNext, onBack, appraisalId }: { data: any, onNext: (data: any) => void, onBack: () => void, appraisalId: string }) {
  ...
  const fetchComps = async () => {
    try {
      ...
      const cleanComps = results.filter(c => c.compType === 'pre_accident')
      setComps(cleanComps)
      if (cleanComps.length > 0) setHasSearched(true)
    } catch (err) {
  ...
  const handleSearch = async () => {
    setIsSearching(true)
    try {
      await triggerComparableSearch(appraisalId, 'pre_accident')
      await fetchComps()
  ...
  return (
    <div className="p-6 sm:p-8">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-medium text-gray-900">Pre-Accident Comparables</h2>
          <p className="mt-1 text-sm text-gray-500">Review clean-history comparable vehicles to establish the Fair Market Value (FMV).</p>
        </div>
        {!hasSearched && (
          <button
      ...
      <div className="mt-8">
        {isSearching ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="mt-2 text-sm text-gray-500">We are scanning Autotrader, Cars.com, and CarGurus for clean-history matches.</p>
          </div>
        ) : comps.length > 0 ? (
          ...
          <div className="rounded-xl border-2 border-dashed border-gray-300 p-12 text-center">
            ...
            <p className="mt-1 text-sm text-gray-500">Click the search button above to find clean-history vehicles.</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-12 flex justify-between">
        ...
        >
          Continue to Post-Accident Comps
        </button>
      </div>
